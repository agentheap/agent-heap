...
<p align="center">
  <a href="https://github.com/heapchain/agent-heap/blob/master/tests/test_agent.py"><img src="https://img.shields.io/badge/tests-14%20passing-brightgreen?style=for-the-badge&logo=pytest" alt="Tests"></a>
  ...
  <a href="https://github.com/heapchain/agent-heap/tree/master/agent/nodes/analyzer.py"><img src="https://img.shields.io/badge/NVIDIA%20NIM-integrated-76B900?style=for-the-badge&logo=nvidia" alt="NVIDIA NIM"></a>
</p>

**Agent Heap** is a LangGraph-powered agent that continuously monitors DeFi lending protocols (Aave, Compound, Morpho) on Arbitrum, analyzes yields via LLM reasoning (NVIDIA NIM), and executes deposits using Kelly-criterion position sizing — all autonomously.
...
- **LLM analysis** — NVIDIA NIM (Mistral/Llama) ranks pools by risk-adjusted return; falls back to max-APY heuristic
...
Environment: `NVIDIA_NIM_API_KEY` (recommended), `ARBITRUM_RPC`, `PRIVATE_KEY`, `DATABASE_URL`.
...
(DeFiLlama)  (NVIDIA    (risk      (Web3.py)
              NIM +      engine)
              ChromaDB)
```

- **Collector** — Fetches live APY + TVL from DeFiLlama for Aave V3, Compound, Morpho on Arbitrum
- **Analyzer** — Queries NVIDIA NIM for pool ranking; retrieves similar past market conditions from ChromaDB
...
- [x] LangGraph pipeline, data feeds, risk modules, CLI, 14 tests
